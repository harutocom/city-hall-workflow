...
- 市役所の庁内業務（休暇申請などのワークフロー）がすべて紙のアナログな手段で行われており、ここに年間700時間以上の削減余地があった。
- 承認者（課長や部長）のデスクに紙の申請書が山積みになっていた。ひどいものだと3年前の申請書が未承認のまま埋もれており、そもそも「申請しても承認・処理されない」という状態が起きていた。

### 開発の目的

- 休暇申請などの各種ワークフローをWebアプリケーションにすることで、紙の山積みをなくし、申請が放置されずに承認・処理される状態にする。
- 申請内容や承認履歴をデータベースに蓄積することで、将来的な勤怠管理や業務分析の基盤を構築する。
- このシステムを導入することで、年間700時間以上の無駄な業務時間を削減する。

## 2. 技術選定およびその意図

本プロジェクトにおける技術選定の最大の基準は、「現場の課題を解決するプロトタイプ（MVP）を最速で開発すること」と「自分達がモダンなWeb技術を学習・実践すること」の2点です。
厳密な他技術との比較よりも、自身が最もスピーディーに開発を進められ、かつキャッチアップしたい技術スタックを採用しています。

### フロントエンド / バックエンド (フルスタック)

- **使用技術:** Next.js (App Router) + React 19 / TypeScript 5
- **選定意図:** フロントエンドからAPIまでを一つのフレームワークで完結でき、最速で動くものを形にするためにNext.jsを採用しました。

### データベース / ORM

- **使用技術:** Neon (Serverless PostgreSQL) / Prisma 6
- **選定意図:** インフラ構築やデータベース設定の時間を極力削り、アプリケーションの実装（課題解決）に時間を全振りしたかったためです。サーバーレスで即座に使えるNeonと、直感的に型安全なDB操作ができて開発体験が良いPrismaを採用しました。

### 認証 / バリデーション

- **使用技術:** NextAuth.js v4 / Zod + React Hook Form
- **選定意図:** ログイン機能やフォームの入力チェックをゼロから自作すると時間がかかり、セキュリティの不安もあったため、Next.jsの定番ツールを使い、実装しました。
...
│   │   ├── auth/
│   │   │   ├── signup/               # POST（※現在は初期セットアップのため認証チェック無効）
...
```bash
# 1. リポジトリのクローン
git clone [https://github.com/harutocom/city-hall-workflow.git](https://github.com/harutocom/city-hall-workflow.git)
